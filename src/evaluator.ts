...
export class Evaluator {
  ...
  evaluateFlag(
    ...
    for (const allocation of flag.allocations) {
      const addUnmatchedAllocation = (code: AllocationEvaluationCode) => {
        unmatchedAllocations.push(createAllocationEvaluation(allocation.key, code));
      };
        ...
        for (const split of allocation.splits) {
          if (
            ...
            const flagEvaluationDetails = flagEvaluationDetailsBuilder
              .setMatch(variation, allocation, matchedRule, unmatchedAllocations)
              .build(
                'MATCH',
  ...
  private getMatchedEvaluationDetailsMessage = (
    ...
    const hasDefinedRules = !!allocation?.rules;
